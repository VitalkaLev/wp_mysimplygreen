			...
			<Fragment>
				<PluginSidebar
					...
					<PanelBody
						...
						onToggle={ () => this.props.toggleColorsPanel() }
						opened={ this.props.colorsPanelOpen } >
...

					{
						...
						<PanelBody
							...
							onToggle={ () => this.props.toggleFontsPanel() }
							opened={ this.props.fontsPanelOpen } >
							<FontPreviews
...
registerPlugin( 'nextgen-site-design', {
	...
	render: compose( [
		...
		withSelect( ( select ) => {
			...
			const { isColorsPanelOpen, isFontsPanelOpen } = select( 'nextgen/site-design' );
			...
			return {
				...
				colorsPanelOpen: isColorsPanelOpen(),
				fontsPanelOpen: isFontsPanelOpen(),
			};
		} ),

		withDispatch( ( dispatch ) => {
			const { updateSettings } = dispatch( 'core/block-editor' );
			const { toggleFontsPanel, toggleColorsPanel } = dispatch( 'nextgen/site-design' );
